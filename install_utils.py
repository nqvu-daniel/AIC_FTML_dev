...
DEPENDENCY_MAP: Dict[str, Tuple[str, Optional[str]]] = {
    ...
    'open_clip_torch': ('open_clip_torch', '>=2.24.0'),
...
def install_package(package_name: str, version: Optional[str] = None, force_gpu: bool = False) -> bool:
    ...
    try:
        ...
        if package_name == 'torch' and package_name in SPECIAL_PACKAGES:
            ...
            logger.info(f"Installing PyTorch ecosystem...")
        else:
...
def smart_install_for_script(script_name: str, force_gpu: bool = False) -> bool:
    ...
    script_dependencies = {
        'smart_pipeline.py': [
            'torch', 'open_clip_torch', 'faiss', 'cv2', 'numpy', 'pandas', 
            'PIL', 'tqdm', 'sklearn', 'scipy', 'pyarrow'
        ],
        'frames_auto.py': [
            'torch', 'open_clip_torch', 'cv2', 'numpy', 'sklearn', 'scipy', 
            'PIL', 'tqdm', 'decord'
        ],
        'search.py': [
            'torch', 'open_clip_torch', 'faiss', 'rank_bm25', 'numpy', 
            'pandas', 'joblib'
        ],
        'train_reranker.py': [
            'torch', 'open_clip_torch', 'faiss', 'sklearn', 'joblib'
        ],
        'train_reranker_gbm.py': [
            'torch', 'open_clip_torch', 'faiss', 'sklearn', 'joblib', 'lightgbm'
        ]
    }
    ...
    if base_name in script_dependencies:
        ...
        core_deps = [
            'torch', 'open_clip_torch', 'faiss', 'cv2', 'numpy', 
            'pandas', 'PIL', 'tqdm', 'sklearn'
        ]
...
if __name__ == "__main__":
    ...
    if args.script:
        ...
        core_deps = ['torch', 'open_clip_torch', 'faiss', 'cv2', 'numpy', 'pandas', 'PIL', 'tqdm']
