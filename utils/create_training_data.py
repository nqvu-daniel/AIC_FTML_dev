...
def create_training_data_from_metadata(dataset_root: str, output_file: str = "data/train.jsonl", num_examples: int = 50):
    ...
    # If we don't have the metadata locally, try to find it in tmp_download or data/dataset_metadata
    if not media_info_dir.exists():
        # Try multiple possible locations
        possible_locations = [
            Path("tmp_download"),  # Legacy location
            Path("data/dataset_metadata"),  # New organized location
        ]
        
        tmp_media_info = None
        tmp_map_keyframes = None
        
        for location in possible_locations:
            media_info_candidate = location / "media-info"
            map_keyframes_candidate = location / "map-keyframes"
            if media_info_candidate.exists() and map_keyframes_candidate.exists():
                tmp_media_info = media_info_candidate
                tmp_map_keyframes = map_keyframes_candidate
                break 
        
        if tmp_media_info and tmp_map_keyframes and tmp_media_info.exists() and tmp_map_keyframes.exists():
